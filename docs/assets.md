...
2. **The repo's people imagery appears to be AI-generated and shows different people.** The man in `family-field.jpeg`, `v-couple.mp4` and `v-family.mp4` (bald, clean-shaven, linen suit) is visibly not the father in the AR shoot (cap, white shirt). The `uploads/` filenames confirm AI origin for the monogram (`ChatGPT_Image_…`) and the videos (prompt-style names). **Recommendation:** use only real AR photos for people, and treat AI imagery as objects and atmosphere only (shoes, rings, sky, florals). **Needs confirmation.**
3. **No real ring or church photography exists.** Rings are rendered in 3D. For the church, the only source is the AI video `v-church.mp4`, which may not depict the real St. Clement's. `TODO(content)`: a real photo of the church, or the couple's approval to use a generic, non-literal architecture treatment.
...
| `assets/family-field.jpeg` | image | 1536×1024 | 3:2 | 246 KB | Family seated in a meadow with a flower basket (the man doesn't match the AR shoot) | faces centre | — (replace with AR) | — | — |
