...
- **Lite tier (`low`; every Android phone, low power):** no WebGL haze; the canvas exists only near The Promise and draws only while a step is moving/settling (30 fps, `frameloop="demand"`), standard-metal rings, DPR 1; never make a layer larger than the screen (scale a small one instead); no animated blurs (use `blurred()`/`focused()` from `animation/fx.ts`, never a raw `filter: blur()` tween); no full-screen blend modes or big filters (override under `html[data-tier="low"]`). Check with `QA_DEVICE=android QA_GL=soft node scripts/qa-stepping.mjs` (compare before/after).
...
## Open items
See `docs/content.md` → "Still open". Deployed at https://helsonandluna.vercel.app (Vercel project `helsonandluna`, auto-deploys from `digisyn-co/helsonandluna` main; `vercel.json` pins the Next.js preset). RSVPs go to a Google Sheet via Apps Script; see `integrations/rsvp-sheet/README.md`. Hand-over notes: `docs/HANDOVER.md`.
